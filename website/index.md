...
(*) - This was real data from Nepenthes 2.0 that I added more examples
to as versions/features progressed.
...
generated by the configured word list. 57 requests were given a delayed
302 to somewhere else in the tarpit.
...
All possible directives in config.yaml:
 ...
 - #### silos
   ...
   - #### redirect_rate
		Optional. Set between 1 and 100, will turn that percentage of
		requests into a 302 redirect elsewhere the tarpit. This is a
		great way to waste time without wasting your CPU or bandwidth;
		'100' can be particularly amusing, as it causes an infinite
		chain of redirects that never complete.
		
		New in v2.5. Defaults to zero - never use 302 redirects.
...
- #### v2.5:
  New per-silo redirect_rate setting
